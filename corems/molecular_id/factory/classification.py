...
class HeteroatomsClassification(Mapping):
    ...
    * mz_exp_all().
    ...
    def plot_van_krevelen(
        self, classe=None, max_hc=2.5, max_oc=2, ticks_number=5, color="viridis", alpha=0.5, 
        log_abundance=False
    ):
        ...
        if classe is not None and classe != Labels.unassigned:
            ...
            # Sort by abundance (ascending so higher abundances are plotted last)
            ...
            # Plot data as scatter plot with color mapped to abundance
            scatter = ax.scatter(oc, hc, c=abundance, alpha=alpha, cmap=color)
            plt.colorbar(scatter, label=colorbar_label)
            
            # Set title with class name and abundance percentage
            title = "%s, %.2f %%" % (classe, abun_perc)
            ax.set_title(title)
            
            # Return axes and abundance percentage
            return_val = ax, abun_perc
        else:
            # All classes plot - use existing all-classes methods
            ...
            # Set title for all classes plot
            ax.set_title("Van Krevelen Diagram - All Assigned Classes")
            
            # Return just the axes
            return_val = ax
        
        # Common settings for both single class and all classes
    ...
    def plot_dbe_vs_carbon_number(
        self, classe=None, max_c=50, max_dbe=40, dbe_incr=5, c_incr=10, color="viridis", alpha=0.5,
        log_abundance=False
    ):
